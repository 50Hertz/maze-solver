...
public class Maze {
    ...
    public Maze(@NonNull String initialMaze) {
        initializeMaze(initialMaze);
    }
    ...
    private void initializeMaze(String text) {
        ...
        int startCount = StringUtils.countOccurrencesOf(text.toUpperCase(), "S");
        int endCount = StringUtils.countOccurrencesOf(text.toUpperCase(), "E");

        Assert.isTrue(startCount == 1, "Only one start/entry point is allowed.");
        Assert.isTrue(endCount == 1, "Only one end/exit point is allowed.");
        ...
        Assert.isTrue(maze.length == maze[0].length, String.format("Please provide a square (n x n) matrix. Height: %d. Width: %d", maze.length, maze[0].length));
    }
    ...
    public String getPath(List<Coordinate> path) {
